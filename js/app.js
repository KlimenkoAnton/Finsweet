...
function formsFieldsInit(options = { viewpass: false, maskTel: false }) {
  document.body.addEventListener("focusin", function (e) {
    const targetElement = e.target;
  ...
  validateInput(input) {
    ...
    if (input.dataset.required === "tel") {
      if (input.value.length < input.getAttribute("maxlength")) {
        forms_formValidate.errorAddBlock(input, `Enter a correct phone number`);
    ...
    if (input.dataset.maxLength) {
      if (input.value.length > input.dataset.maxLength) {
        forms_formValidate.errorAddBlock(input, `Maximum number of characters ${input.dataset.maxLength}`);
        forms_formValidate.addErrorClass(input);
        error++;
      }
    }
    if (input.dataset.minLength) {
      if (input.value.length < input.dataset.minLength) {
        forms_formValidate.errorAddBlock(input, `Minimum number of characters ${input.dataset.minLength}`);
        forms_formValidate.addErrorClass(input);
        error++;
      }
    }
    if (input.dataset.required === "email") {
      if (forms_formValidate.emailTest(input)) {
        forms_formValidate.errorAddBlock(input, `Please enter correct email`);
        forms_formValidate.addErrorClass(input);
        error++;
      }
    } else if (input.getAttribute("type") === "checkbox" && input.checked === false) {
    ...
    if (input.value === "") {
      forms_formValidate.errorAddBlock(input, "Fill in this field");
      forms_formValidate.addErrorClass(input);
      error++;
    }
    return error;
...
function initSliders() {
  ...
  if (document.querySelector('.slider-testimonials')) { //Указываем класс нужного слайдера
    //Создаем слайдер
    new Swiper('.slider-testimonials__slider', { //Указываем класс нужного слайдера
      ...
      */
      ...
      // Кнопки "влево/вправо"
      navigation: {
        prevEl: '.slider-testimonials .arrows-slider__arrow_prev',
        nextEl: '.slider-testimonials .arrows-slider__arrow_next',
      },
